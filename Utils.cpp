...
Updates:
15/04/2012
- Put in hack to deal with no space at the end of a line in Tokenize 
30/04/2012
- Copied into fit_my_ecp; disabled anything related to 2D and 3D searches
- Added clock timer to see how long IO takes
**/
...
{	
	// Horrible hack to deal with no space left at the end of a line
	string text = "";
	string value = "";
	//cout << str << endl;
	...
	// This does the job but long term it'd be nice to have something better
	
	string::size_type lastPos = text.find_first_not_of(delimiters, 0); // Skip delimiters at beginning.
 	string::size_type pos     = text.find_first_of(delimiters, lastPos); // Find first "non-delimiter".
	
 	while (string::npos != pos || string::npos != lastPos)
 	{
 		tokens.push_back(text.substr(lastPos, pos - lastPos)); // Found a token, add it to the vector.
 		lastPos = text.find_first_not_of(delimiters, pos); // Skip delimiters.  Note the "not_of"
 		pos = text.find_first_of(delimiters, lastPos); // Find next "non-delimiter"
 	}
}
/**
...
// Lets use this to benchmark how long everything takes in milliseconds
// Lifted from http://en.cppreference.com/w/cpp/chrono/c/clock
{
...
{	
	// return range [0..hi-1]
	return double(randomNumber(idum)*hi); // implicit cast and truncation in return
}
